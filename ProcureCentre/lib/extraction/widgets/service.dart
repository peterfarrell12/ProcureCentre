...
Future<bool> sendFile(List<String> file, List<String> filenames) async {
 ...
 if (responses.toSet().contains(401)) {
   return false;
 }
...
Future<List<DataPoint>> retrieveData(
  ...
  for (int k = 0; k < noInvoices; k++) {
    ...
    var items =
        j['results'][k]['content'][5]['children'][0]['children'] as List;
    for (var i = 0; i < items.length; i++) {
      ...
      DataPoint test = DataPoint(
        ...
        price: amount_base.isNotEmpty
            ? amount_base
            : (double.parse(amount_total) / double.parse(quantity)).toString(),
